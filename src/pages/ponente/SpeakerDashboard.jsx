...
export default function SpeakerDashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#0B1310] flex flex-col font-sans">
        {/* Skeleton Header */}
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F2F5F3] dark:bg-[#0B1310] flex flex-col font-sans relative overflow-hidden">
        ...
        <div className="bg-white dark:bg-[#122A1C] border-b border-gray-100 dark:border-emerald-900/30 sticky top-0 z-20 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 flex gap-8 overflow-x-auto scrollbar-hide">
            {[
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`py-5 px-1 border-b-4 font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2.5 whitespace-nowrap ${activeTab === tab.id ? 'border-[#0F5B3C] dark:border-emerald-500 text-[#0F5B3C] dark:text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>{tab.icon && <tab.icon size={16} />} {tab.label} {tab.count > 0 && <span className="bg-red-500 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] animate-pulse">{tab.count}</span>}</button>
            ))}
          </div>
        </div>
            ...
            <div className="space-y-10 anim-fade-up">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-[#122A1C] p-8 rounded-[2.5rem] shadow-xl border border-white dark:border-emerald-900/20 relative overflow-hidden group">
                  ...
                  <div className="w-14 h-14 bg-emerald-50 dark:bg-emerald-900/40 rounded-2xl flex items-center justify-center text-[#0F5B3C] dark:text-emerald-400 mb-6 group-hover:scale-110 transition-transform"><Users size={28} /></div>
                ...
                <div className="bg-[#0F5B3C] dark:bg-emerald-900 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group flex flex-col justify-center items-center text-center">
                  <div className="absolute inset-0 bg-gradient-to-tr from-[#0B1310] to-transparent opacity-50" />
                  <MonitorPlay size={40} className="text-emerald-400 mb-4 relative z-10" />
                  <button onClick={() => setPresentationMode(true)} className="relative z-10 w-full py-4 bg-white text-[#0F5B3C] hover:bg-emerald-50 rounded-[1.5rem] font-black uppercase text-xs tracking-widest shadow-2xl transition-all active:scale-95">Modo Escenario</button>
                      ...
                      Object.entries(sesion.stats_carreras).map(([carrera, count]) => {
                        ...
                        return (
                          <div key={carrera} className="space-y-2">
                            <div className="flex justify-between items-center text-xs font-black uppercase tracking-widest"><span className="text-gray-500">{PROGRAMA_LABELS[carrera] || carrera}</span><span className="text-[#0F5B3C] dark:text-emerald-400">{count} alumnos ({Math.round(pct)}%)</span></div>
                            <div className="h-3 bg-gray-50 dark:bg-black/40 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${PROGRAMA_COLORS[carrera] || 'bg-[#0F5B3C]'}`} style={{ width: `${pct}%` }} /></div>
                          </div>
                        )
                      })
                    )}
            ...
            <div className="bg-white dark:bg-[#122A1C] rounded-[3rem] shadow-2xl border border-white dark:border-emerald-900/20 overflow-hidden anim-fade-up">
              <div className="p-8 sm:p-10 border-b border-gray-100 dark:border-emerald-900/30 flex justify-between items-center bg-gray-50/50 dark:bg-black/20">
                ...
                <div className="flex gap-4">
                  ...
                  <span className="bg-[#0F5B3C] text-white px-5 py-2 rounded-2xl text-xs font-black shadow-lg shadow-emerald-900/20">{preguntas.length} TOTALES</span>
                </div>
              </div>
                ...
                {preguntas.length === 0 ? (
                  ...
                  preguntas.map((p) => (
                    <div key={p.id} className={`p-8 flex flex-col sm:flex-row gap-6 sm:items-center justify-between transition-all ${p.estado === 'respondida' ? 'opacity-40 grayscale bg-gray-50 dark:bg-black/20' : 'hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10'}`}>
                      ...
                      {p.estado === 'pendiente' && ( <button onClick={() => updatePreguntaEstado(p.id, 'respondida')} className="shrink-0 flex items-center justify-center gap-2 px-6 py-3 bg-[#0F5B3C] hover:bg-emerald-800 text-white rounded-2xl text-xs font-black shadow-xl shadow-emerald-950/20 active:scale-95"><Check size={16} /> LISTO</button> )}
                    </div>
                  ))
                )}
              </div>
          ...
          {activeTab === 'polls' && (
            <div className="max-w-3xl mx-auto anim-fade-up">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setShowPollModal(true)} className="flex items-center gap-2 px-6 py-3 bg-[#0F5B3C] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95"><Plus size={16} /> Crear Encuesta</button>
              </div>
            ...
            <div className="max-w-4xl mx-auto anim-fade-up">
              <div className="bg-white dark:bg-[#122A1C] rounded-[3rem] p-10 shadow-2xl border border-white dark:border-emerald-900/20">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                  ...
                  <button onClick={downloadCSV} disabled={networkingLeads.length === 0} className="flex items-center gap-2 px-6 py-3 bg-[#0F5B3C] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl disabled:opacity-50 active:scale-95"><Download size={16} /> Exportar CSV</button>
                </div>
          ...
          {activeTab === 'scan' && (
            <div className="max-w-lg mx-auto anim-fade-up">
              <div className="bg-white dark:bg-[#122A1C] rounded-[3rem] shadow-2xl border-4 border-white dark:border-emerald-900/30 p-10 text-center">
                <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-900/30 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-inner"><ScanLine size={32} className="text-[#0F5B3C] dark:text-emerald-400" /></div>
                ...
                <div className="relative rounded-[2.5rem] overflow-hidden bg-black aspect-square mb-10 border-8 border-gray-50 dark:border-black shadow-2xl">
                  {!scanning ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-8"><Sparkles size={48} className="text-emerald-500/20 mb-6" /><button onClick={() => setScanning(true)} className="w-full py-4 bg-[#0F5B3C] text-white rounded-[1.5rem] font-black uppercase text-xs tracking-widest shadow-xl shadow-emerald-950/20">Iniciar Escáner</button></div>
                  ) : (
                    <>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-[#0F2018] border border-gray-100 dark:border-emerald-900/50 rounded-2xl outline-none focus:border-[#0F5B3C] text-sm font-bold text-gray-900 dark:text-gray-100"
                />
              </div>
              ...
              <div>
                  ...
                  {pollOptions.map((opt, i) => (
                    <div key={i} className="flex gap-2">
                      <input 
                        ...
                        className="flex-1 px-4 py-3 bg-gray-50 dark:bg-[#0F2018] border border-gray-100 dark:border-emerald-900/50 rounded-xl outline-none focus:border-[#0F5B3C] text-sm font-semibold text-gray-900 dark:text-gray-100"
                      />
                ...
                {pollOptions.length < 5 && (
                  <button 
                    ...
                    className="mt-4 text-xs font-bold text-[#0F5B3C] dark:text-emerald-400 uppercase tracking-widest flex items-center gap-1 hover:opacity-70 transition-opacity"
                  >
              ...
              <button 
                ...
                className="w-full py-4 mt-4 bg-[#0F5B3C] text-white font-black uppercase text-xs tracking-widest rounded-2xl hover:bg-emerald-800 transition-all shadow-lg shadow-emerald-900/20 disabled:opacity-50"
              >
