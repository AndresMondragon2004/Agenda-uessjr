    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="bg-white dark:bg-[#122A1C] w-full max-w-lg max-h-[95vh] flex flex-col rounded-[2.5rem] overflow-hidden shadow-2xl animate-scale-in">
        <div className="p-8 border-b border-gray-100 dark:border-emerald-900/40 flex items-center justify-between shrink-0">
          <div>
            <h3 className="font-black text-xl text-[#22573E] dark:text-emerald-400 uppercase">Encuadre del Programa</h3>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input type="range" min="0" max="100" value={offset.x} onChange={(e) => setOffset(p => ({...p, x: e.target.value}))} 
                className="w-full h-1.5 bg-gray-100 dark:bg-emerald-900/40 rounded-lg appearance-none cursor-pointer accent-[#22573E]" />
            </div>
        ...
        <div className="p-8 bg-gray-50 dark:bg-emerald-900/20 flex gap-4 shrink-0">
          ...
          <button onClick={handleSave} disabled={loading} className="flex-2 px-10 py-4 bg-[#22573E] text-white font-black uppercase text-xs tracking-widest rounded-2xl hover:bg-emerald-800 shadow-xl shadow-emerald-950/20 transition-all flex items-center justify-center gap-2">
...
const EXPORT_ROWS = [
  ...
  const handleUploadHeaderLogo = async (e) => {
    ...
    if (logosEncabezado.length >= 2) {
      showToast('Solo se permiten cargar hasta 2 logos para el encabezado', 'error')
      return
    }
    ...
    try {
      ...
      cargarDatos()
      showToast('Logo de encabezado añadido correctamente')
    } catch (err) {
      showToast('Error al subir logo: ' + err.message, 'error')
    } finally {
      setLoading(false)
    ...
    <>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-24">
            <div className="text-center">
              <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#22573E] mx-auto mb-3" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              <div className="bg-white dark:bg-[#122A1C] rounded-2xl shadow-sm p-8 border border-gray-100 dark:border-emerald-900/40">
                <div className="flex items-center gap-3 mb-1">
                  <div className="p-2 bg-emerald-50 dark:bg-emerald-900/40 rounded-lg">
                    <FileText className="w-5 h-5 text-[#22573E] dark:text-emerald-400" />
                  </div>
                    ...
                    <button type="button"
                      ...
                      className={`px-5 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all shadow-sm
                        ${diasSeleccionados === 'todos'
                          ...
                          : 'bg-white dark:bg-[#0F2018] border border-gray-200 dark:border-emerald-900/40 text-gray-600 dark:text-gray-300 hover:border-[#22573E]/30 hover:bg-emerald-50 dark:hover:bg-emerald-900/30'}`}>
                      Evento completo
                    </button>
                    {dias.map(dia => (
                    ...
                    {[
                      ...
                      <label key={i} className="flex items-start gap-3 p-4 rounded-xl border border-gray-100 dark:border-emerald-900/40 hover:border-emerald-100 dark:hover:border-emerald-800/50 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/20 transition-all cursor-pointer group">
                        <div className={`mt-0.5 flex shrink-0 items-center justify-center w-5 h-5 rounded-lg border-2 transition-all
                          ${item.checked ? 'bg-[#22573E] border-[#22573E]' : 'border-gray-200 dark:border-emerald-900/60 bg-white dark:bg-[#0F2018] group-hover:border-[#22573E]/50'}`}>
                        ...
                        <div>
                          <p className="text-sm text-gray-800 dark:text-gray-200 font-bold group-hover:text-[#22573E] dark:group-hover:text-emerald-400 transition-colors">{item.label}</p>
                ...
                <div className="bg-gray-50 dark:bg-[#0F2018] rounded-2xl p-6 mb-8 flex items-center justify-between border border-gray-100 dark:border-emerald-900/40">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-black text-[#22573E] dark:text-emerald-400">{totalPages}</p>
                  </div>
                </div>

                <button type="button" onClick={handleGenerarPDF} disabled={generando}
                  className="w-full py-4 text-white font-black uppercase tracking-widest rounded-2xl bg-[#22573E] hover:bg-[#002F1D] hover:-translate-y-0.5 transition-all shadow-lg shadow-emerald-900/10 disabled:opacity-50 flex items-center justify-center gap-3">
                  {generando ? (
                    <>
                  ...
                  {EXPORT_ROWS.map((row) => (
                    <div key={row.key} className="flex items-center justify-between p-4 border border-gray-50 dark:border-emerald-900/30 rounded-2xl hover:border-emerald-100 dark:hover:border-emerald-800/50 hover:bg-gray-50/50 dark:hover:bg-emerald-900/10 transition-all group">
                      <div className="flex items-start gap-4 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <p className="text-sm text-gray-800 dark:text-gray-200 font-black group-hover:text-[#22573E] dark:group-hover:text-emerald-400 transition-colors">{row.label}</p>
                      ...
                      <button type="button"
                        onClick={row.fn}
                        disabled={!!exportando}
                        className="shrink-0 ml-4 px-5 py-2 text-xs font-black uppercase tracking-wider text-[#22573E] dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl hover:bg-[#22573E] dark:hover:bg-[#22573E] hover:text-white transition-all disabled:opacity-40 min-w-[110px] border border-transparent hover:shadow-md">
                        {exportando === row.key ? (
                ...
                <div className="border-t border-gray-100 dark:border-emerald-900/20 pt-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {logosEncabezado.length < 2 && (
                      <label className="cursor-pointer px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 text-[#22573E] dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#22573E] hover:text-white dark:hover:bg-emerald-700 transition-all flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {logosEncabezado.map((logo, index) => (
                      <div key={logo.id} className="bg-gray-50 dark:bg-[#0F2018] rounded-2xl p-4 text-center group border border-transparent hover:border-red-100 dark:hover:border-red-900/40 transition-all relative">
                        ...
                        <span className="text-[8px] font-bold text-[#22573E] dark:text-emerald-400 block mt-1 bg-[#22573E]/10 dark:bg-emerald-950/40 px-2 py-0.5 rounded">
            ...
            <div className="lg:col-span-2">
              <div className="lg:sticky lg:top-24 space-y-6">
                <div className="bg-white dark:bg-[#122A1C] rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-emerald-900/40 overflow-hidden relative">
                  ...
                  <div className="w-full rounded-2xl overflow-hidden border border-gray-200 shadow-2xl shadow-emerald-900/5 transition-all transform hover:scale-[1.02]"
                    ...
                    {paginaPreview === 1 ? (
                      <div className="w-full h-full flex flex-col p-4" style={{ background: '#F9F7F2' }}>
                        <div className="flex items-center justify-between border-b-2 border-[#22573E] pb-2 mb-4">
                          ...
                          <p className="text-[8px] font-black text-[#22573E] tracking-widest">UES SAN JOSÉ DEL RINCÓN</p>
                          <div className="w-12 h-8 flex items-center justify-center">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center">
                          ...
                          <h4 className="text-sm font-black text-[#22573E] uppercase tracking-tighter leading-none">Jornada Académica</h4>
                          <h4 className="text-sm font-black text-[#22573E] uppercase tracking-tighter leading-none mb-2">y Cultural 2026</h4>
                          <p className="text-[6px] font-black text-gray-400 tracking-[0.3em] mb-4">P R O G R A M A</p>
                          <div className="px-3 py-1 bg-[#22573E] text-white text-[7px] font-black rounded-lg">
                      ...
                      <div className="w-full h-full flex flex-col p-4" style={{ background: '#F9F7F2' }}>
                        <div className="flex items-start justify-between border-b-2 border-[#D4A017] pb-2 mb-4 relative">
                          ...
                          <div className="flex-1 flex flex-col items-center text-center px-2">
                            <h4 className="text-[8px] font-black text-[#22573E] uppercase tracking-wider leading-none">UES SAN JOSÉ DEL RINCÓN</h4>
                            ...
                            <div className="space-y-3">
                              {sesionesPreview.length > 0 ? sesionesPreview.map((ses) => (
                                <div key={ses.id} className="flex gap-4 items-start opacity-80 border-b border-gray-50 pb-2">
                                  <span className="text-[6px] font-black text-[#22573E] w-8">{ses.hora_inicio?.slice(0, 5)}</span>
                                  <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between mt-6 bg-gray-50 dark:bg-[#0F2018] p-2 rounded-2xl border border-gray-100 dark:border-emerald-900/30">
                    <button type="button" disabled={paginaPreview <= 1}
                      ...
                      className="p-2 text-gray-400 dark:text-gray-500 hover:text-[#22573E] dark:hover:text-emerald-400 disabled:opacity-20 transition-all">
                      <ChevronLeft className="w-5 h-5" strokeWidth={3} />
                    </button>
                    <span className="text-[10px] font-black text-[#22573E] dark:text-emerald-400 uppercase tracking-widest">
                ...
                {/* Info banner */}
                {diasSeleccionados !== 'todos' && (
                  <div className="bg-[#22573E] rounded-2xl p-6 text-white shadow-xl shadow-emerald-900/10">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-emerald-950/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#122A1C] w-full max-w-2xl rounded-3xl shadow-2xl border border-emerald-100 dark:border-emerald-900/40 overflow-hidden scale-in-center">
            <div className="p-6 border-b border-gray-100 dark:border-emerald-900/40 flex items-center justify-between bg-gray-50/50 dark:bg-emerald-900/20">
              <div>
                <h3 className="font-black text-lg text-[#22573E] dark:text-emerald-400 uppercase tracking-tight">Imágenes del Programa</h3>
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-emerald-100 scrollbar-track-transparent">
              {dias.map((dia) => (
                <div key={dia.id} className="flex items-center gap-6 p-4 rounded-2xl border border-gray-100 dark:border-emerald-900/40 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-all group">
                  <div className="w-24 h-16 rounded-xl overflow-hidden bg-gray-100 dark:bg-[#0F2018] shrink-0 border border-gray-200 dark:border-emerald-900/40 relative">
                    ...
                    {loadingImagen === dia.id && (
                      <div className="absolute inset-0 bg-white/60 dark:bg-black/60 flex items-center justify-center">
                        <div className="animate-spin rounded-full h-4 w-4 border-2 border-[#22573E] border-b-transparent" />
                      </div>
                    )}
                  </div>
                  ...
                  <label className="cursor-pointer px-4 py-2 bg-white dark:bg-[#0F2018] border border-gray-200 dark:border-emerald-900/60 text-[#22573E] dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-xl hover:border-[#22573E] transition-all flex items-center gap-2 shadow-sm">
            ...
            <div className="p-6 bg-gray-50 dark:bg-emerald-900/20 text-right">
              <button 
                onClick={() => setShowModalImagenes(false)}
                className="px-8 py-3 bg-[#22573E] text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-[#002F1D] shadow-lg shadow-emerald-900/10 transition-all">
                Cerrar Panel
              </button>
            </div>
          </div>
      ...
      {toast && (
        <div className={`fixed bottom-8 left-4 right-4 sm:left-auto sm:right-8 z-50 px-8 py-4 rounded-2xl shadow-2xl text-sm font-black text-white flex items-center gap-3 animate-slide-up
          ${toast.tipo === 'error' ? 'bg-red-600' : 'bg-[#22573E]'}`}>
