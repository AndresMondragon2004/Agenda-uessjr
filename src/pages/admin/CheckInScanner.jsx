...
export default function CheckInScanner() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#05140B] pb-20">
      ...
      <div className="bg-white dark:bg-[#122A1C] border-b border-gray-100 dark:border-emerald-900/40 px-4 py-4 sticky top-0 z-50">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          ...
          <Users size={20} className="text-[#163020]" />
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto px-4 mt-8 space-y-6">
        ...
        <div className="bg-white dark:bg-[#122A1C] rounded-[2rem] p-6 border border-gray-100 dark:border-emerald-900/30">
          ...
          <button disabled={!sesionId} onClick={() => setScanning(!scanning)} className={`w-full py-4 mt-4 rounded-2xl font-black text-sm uppercase transition-all flex items-center justify-center gap-3 ${scanning ? 'bg-red-50 text-red-600' : 'bg-[#163020] text-white'}`}>
        ...
        <div ref={cameraAreaRef} className="scroll-mt-24">
          {scanning && sesionId && (
            <div className="relative anim-fade-up">
              ...
              <div className="absolute inset-x-0 -bottom-3 flex justify-center"><span className="bg-[#163020] text-white px-4 py-1 rounded-full text-[9px] font-black uppercase">Cámara Activa</span></div>
            </div>
          )}
        </div>
        ...
        {scanning && !lastResult && sesionId && (
          <div className="bg-gray-100 dark:bg-[#122A1C] p-6 rounded-[2rem] text-center anim-fade-up">
            <h2 className="text-sm font-black text-[#163020]">{sesiones.find(s => s.id === sesionId)?.nombre}</h2>
