...
// ─── Escenario Card ───────────────────────────────────────────────────────
function EscenarioCard({ esc, onEdit, onDelete, onToggleDisponible, sesionesAsignadas = 0 }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm p-6 flex flex-col gap-4">
      <div className="flex items-start justify-between gap-2">
        <h3 className="font-bold text-gray-900 text-lg leading-tight">{esc.nombre}</h3>
        <span className={`shrink-0 px-3 py-1 rounded-full text-xs font-semibold
          ${esc.disponible ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
          {esc.disponible ? 'Disponible' : 'No disponible'}
        </span>
      </div>

      <p className={`text-sm ${esc.descripcion ? 'text-gray-500' : 'text-gray-300 italic'}`}>
        {esc.descripcion || 'Sin descripción'}
      </p>

      <div>
        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold capitalize
          ${TIPO_STYLES[esc.tipo] || 'bg-gray-100 text-gray-600'}`}>
          {esc.tipo ? esc.tipo.charAt(0).toUpperCase() + esc.tipo.slice(1) : '—'}
        </span>
      </div>

      <div>
        <p className="text-xs text-gray-400 uppercase tracking-wide mb-1">Capacidad máxima</p>
        <div className="flex items-baseline gap-1.5">
          <span className="text-4xl font-bold text-[#004F31]">{esc.capacidad_maxima}</span>
      ...
      <div>
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          ...
          <span>{sesionesAsignadas} / {esc.capacidad_maxima}</span>
        </div>
        <div className="bg-gray-100 rounded-full h-1.5 overflow-hidden">
          <div
            className={`rounded-full h-1.5 transition-all duration-500 ${(() => {
              const pct = esc.capacidad_maxima ? (sesionesAsignadas / esc.capacidad_maxima) * 100 : 0
              return pct >= 100 ? 'bg-red-500' : pct >= 80 ? 'bg-amber-500' : 'bg-[#004F31]'
            })()}`}
            style={{ width: `${esc.capacidad_maxima ? Math.min((sesionesAsignadas / esc.capacidad_maxima) * 100, 100) : 0}%` }}
          />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 min-w-0">
          <button
            type="button"
            onClick={() => onToggleDisponible(esc)}
            title={esc.disponible ? 'Marcar como no disponible' : 'Marcar como disponible'}
            className={`relative w-11 h-6 rounded-full transition-colors shrink-0
              ${esc.disponible ? 'bg-[#004F31]' : 'bg-gray-300'}`}
          >
            <span className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform shadow
              ${esc.disponible ? 'translate-x-5' : 'translate-x-0'}`} />
          </button>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => onEdit(esc)} title="Editar"
                  className="p-2 text-gray-400 border border-gray-200 rounded-xl
                             hover:bg-emerald-50 hover:text-[#004F31] hover:border-[#004F31]/30 transition-all">
            <Edit2 className="w-4 h-4" />
          </button>
          <button type="button" onClick={() => onDelete(esc)} title="Eliminar"
                  className="p-2 text-gray-400 border border-gray-200 rounded-xl
