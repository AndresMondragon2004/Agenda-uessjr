...
export default function ScenariosManagement() {
  ...
  const handleSaved      = () => { handleCloseModal(); cargarEscenarios(); toast.success('Escenario guardado exitosamente') }
  ...
  const handleToggleDisponible = async (esc) => {
    try {
      ...
      cargarEscenarios()
      toast.success(esc.disponible ? 'Marcado como no disponible' : 'Marcado como disponible')
    } catch (err) {
      setError(err.message)
    }
  }
        ...
        {showBanner && (
          <div className="bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800/50 rounded-2xl p-6 mb-8 flex items-center justify-between gap-4 shadow-sm group">
            <div className="flex items-center gap-4">
              ...
              <p className="text-xs font-bold text-[#163020] dark:text-emerald-100 leading-relaxed">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  <div key={esc.id} className="bg-white dark:bg-[#122A1C] rounded-[32px] p-8 border border-gray-100 dark:border-emerald-900/40 shadow-sm hover:shadow-2xl hover:shadow-emerald-900/5 transition-all flex flex-col group relative overflow-hidden">
                    ...
                    <div className="flex items-start justify-between mb-6">
                      <div className="space-y-1">
                        ...
                        <h3 className="text-xl font-black text-gray-900 dark:text-gray-100 group-hover:text-[#163020] dark:group-hover:text-emerald-400 transition-colors">{esc.nombre}</h3>
                      </div>
                      <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm
                        ${esc.disponible ? 'bg-[#163020] text-white' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                    ...
                    <div className="bg-gray-50/80 dark:bg-[#0F2018]/80 rounded-2xl p-4 mb-8 flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-3xl font-black text-[#163020] dark:text-emerald-400 leading-none">{esc.capacidad_maxima}</p>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-gray-50 dark:border-emerald-900/30 mt-auto">
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          className="p-2.5 text-gray-400 dark:text-gray-500 border border-gray-100 dark:border-emerald-900/40 rounded-xl hover:text-[#163020] dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:border-emerald-100 dark:hover:border-emerald-800/50 transition-all"
                        >
              ...
              >
                <div className="w-16 h-16 bg-white dark:bg-[#122A1C] rounded-3xl flex items-center justify-center text-gray-200 dark:text-gray-700 group-hover:text-[#163020] dark:group-hover:text-emerald-400 group-hover:shadow-xl transition-all">
                  <Plus className="w-8 h-8" strokeWidth={3} />
                </div>
                <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest group-hover:text-[#163020]">Añadir nuevo escenario</p>
              </button>
            </div>
