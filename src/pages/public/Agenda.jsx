...
import { Clock, MapPin, Search, Download, ChevronRight, Loader2, Share2, Check, ThumbsUp } from 'lucide-react'
  ...
  useEffect(() => {
    async function cargar() {
      try {
        ...
        // Cargar votos
        try {
          const counts = await votosService.getAllCounts()
          setVotosMap(counts)
        } catch (vErr) {
          console.error("Error cargando votos:", vErr)
        }
      } catch (err) {
        console.error(err)
      } finally {
  ...
  return (
    <div className="min-h-screen bg-[#F5F4F0] dark:bg-[#05140B]">
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8 sm:py-12 max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              {!loading && (
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                  ...
                  {progFiltro !== 'todos' && (
                    <span> · Filtrando por <span className="font-semibold text-[#163020] dark:text-emerald-400">{PROGRAMA_LABELS[progFiltro]}</span></span>
                  )}
                </p>
              )}
            </div>
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 border-2 border-[#163020] dark:border-emerald-700 text-[#163020] dark:text-emerald-400 font-semibold rounded-xl hover:bg-[#163020] dark:hover:bg-emerald-800 hover:text-white transition-all text-sm min-h-[44px] shrink-0 disabled:opacity-50"
            >
              {generating ? (
                <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* FILTER BAR */}
        <div className="bg-[#FAF9F6] dark:bg-[#122A1C] rounded-2xl shadow-sm border border-gray-100 dark:border-emerald-900/40 p-4 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[11px] font-bold whitespace-nowrap transition-all border-2 ${
                      diaFiltro === 'todos'
                        ? 'bg-[#163020] text-white border-[#163020] shadow-md'
                        : 'bg-white dark:bg-[#0F2018] text-gray-500 dark:text-gray-400 border-gray-100 dark:border-emerald-900/30 hover:border-[#163020] dark:hover:border-emerald-700'
                    }`}
                  >
                    Todos
                  </button>
              ...
              <div className="md:col-span-3">
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-gray-100 dark:border-emerald-900/50 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-300 focus:border-[#163020] outline-none bg-gray-50/50 dark:bg-emerald-950/20"
                >
              ...
              <div className="md:col-span-5 relative">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 border border-gray-100 dark:border-emerald-900/50 rounded-xl text-xs font-bold focus:border-[#163020] outline-none bg-gray-50/50 dark:bg-emerald-950/20 dark:text-gray-300"
                  />
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-white dark:bg-[#122A1C] rounded-2xl border border-gray-100 dark:border-emerald-900/40">
            ...
            {sesiones.length > 0 && (
              <button
                onClick={() => { setDiaFiltro('todos'); setTipoFiltro('todos'); updateProgFiltro('todos'); setBusqueda('') }}
                className="mt-4 px-5 py-2 bg-[#163020] text-white font-semibold rounded-xl text-sm hover:bg-emerald-800 transition-colors"
              >
            ...
            {sesionesAgrupadas.map(({ dia, sesiones: sesDia }) => (
              <div key={dia.id}>
                ...
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <div className="flex items-center gap-2 bg-[#163020] text-white px-4 py-1.5 rounded-full">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {ses.ponente_nombre && (
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-[#163020]/10 dark:bg-emerald-900/50 flex items-center justify-center shrink-0">
                                <span className="text-[#163020] dark:text-emerald-400 text-[10px] font-bold">
                        ...
                        <div className="shrink-0 flex flex-col gap-2 sm:min-w-[140px]">
                          <div className="flex gap-2">
                            <Link
                              to={`/agenda/${ses.id}`}
                              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 bg-[#163020] text-white text-xs font-bold rounded-xl hover:bg-emerald-800 transition-colors min-h-[42px] shadow-sm"
                            >
                              Detalles <ChevronRight size={13} />
                            </Link>
                            <button
                              onClick={(e) => { e.preventDefault(); handleShare(ses.id) }}
                              className="px-3 py-2.5 bg-gray-100 dark:bg-emerald-900/30 text-gray-500 dark:text-gray-400 hover:text-[#163020] dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/50 rounded-xl transition-all min-h-[42px]"
